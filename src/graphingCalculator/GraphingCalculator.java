...
public class GraphingCalculator {
    ...
    public GraphingCalculator(int width, int height) {
        ...
        coordinatesChanged();
         ...
         AtomicBoolean animating = new AtomicBoolean(true);

        canvas.onDrag(event ->
                setAnimationParameter(
                        getAnimationParameter() + event.getDelta().getX() / width));

        canvas.onMouseDown(event -> animating.set(false));
        canvas.onMouseUp(event -> animating.set(true));

        canvas.animate(() -> { if (animating.get()){
                setAnimationParameter(
